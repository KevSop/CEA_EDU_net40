...
namespace CEA_EDU.Domain.Manager
{
    ...
    public class DicManager : ManagerBase
    {
        ...
        public List<DicEntity> GetSearch(string keyString, string sort, string order, int offset, int pageSize, out int total)
        {
            ...
            int pageCount = 0;
            string querySql = string.Format("select * from SysDic where iIsDeleted =0 and iStatus =1 and iKey like '%{0}%' ", keyString);
            ...
            List<DicEntity> list = new List<DicEntity>();
            foreach (DataRow dr in dt.Rows)
            {
                DicEntity entity = new DicEntity();

                entity.iId = Ext.ToString(dr["iId"]);
                entity.iType = Ext.ToString(dr["iType"]);
                entity.iKey = Ext.ToString(dr["iKey"]);
                entity.iValue = Ext.ToString(dr["iValue"]);
                entity.iCreatedOn = Ext.ToDate(dr["iCreatedOn"]);
                entity.iCreatedBy = Ext.ToString(dr["iCreatedBy"]);
                entity.iUpdatedOn = Ext.ToDate(dr["iUpdatedOn"]);
                entity.iUpdatedBy = Ext.ToString(dr["iUpdatedBy"]);
                entity.iStatus = Ext.ToInt(dr["iCreatedOn"]);
                entity.iIsDeleted = Ext.ToInt(dr["iIsDeleted"]);
